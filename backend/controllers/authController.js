...
const updateUserProfile = async (req, res) => {
    try {
        ...
        res.json({ id: updatedUser.id, name: updatedUser.name, email: updatedUser.email, gender: updatedUser.gender, phone: updatedUser.phone, dateOfBirth: updatedUser.dateOfBirth, token: generateToken(updatedUser.id) });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
